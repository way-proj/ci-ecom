   ...
   public function save_blog_data(){
    ...
    $this->load->library('upload');
      ...
      $file_upload_name="";
				...
				{
			     $file_upload_name=$this->blog_update_do_upload();
				}
...
public function save_company_data(){
	...
	/*public function save_brand_data(){
		...
		$uploadStatus="";
		$file_upload_name="";
			if(isset($_FILES['update_photo']) && $_FILES['update_photo']['name'] != '')
				{
				    $config=array();
				    $config['upload_path'] = getcwd().'/upload/cat_images/';
            ...
            $this->form_validation->set_rules('update_photo', 'photo image','required',array('required'=>$uploadStatus));
			...
			//brand logo
			$file_upload_name1="";
			if(isset($_FILES['update_photo1']) && $_FILES['update_photo1']['name'] != '')
				{
				$config=array();
				$config['upload_path'] = getcwd().'/upload/cat_images/';
				$config['allowed_types'] = 'jpg|png|jpeg|gif|webp';
					...
					if($this->upload->do_upload('update_photo1'))
					{
						$upload_data = $this->upload->data();
					...
					$this->form_validation->set_rules('update_photo1', 'Brand logo','required',array('required'=>$uploadStatus));
			...
			//$this->db->update('brand_details',$update_data);
       ...
       //$this->db->insert('brand_details',$insert_data);
		  $result=$this->Product_model->save_brand_data($insert_data);
          redirect(base_url().'Productmaster/brand_list');
 
     }
     redirect(base_url().'Productmaster/brand_list',$data);
...
public function save_brand_data(){
		...
		//$file_upload_name="";
            ...
            $this->form_validation->set_rules('file', 'photo image','required',array('required'=>$uploadStatus));
			...
			if(isset($_FILES['file1']) && $_FILES['file1']['name'] != '')
				...
				$config['allowed_types'] = 'jpg|png|jpeg|gif|webp';
					...
					$this->form_validation->set_rules('file1', 'Brand logo','required',array('required'=>$uploadStatus));
...
public function save_sub_category_data(){
    ...
    if($sub_cat_id){
         ...
         if($file!=''){ 
          ...
          'meta_keyword'=> $this->input->post("meta_keyword"),
          'feature_category'=>$this->input->post("feature_category"),
          'store'=>$this->input->post("brandname"),
          'sort_order'=>$this->input->post("sort_order"),
          'status'=> $this->input->post("status"),
          'top'=> $this->input->post("top"),
          'image_url'=> $upload_file,
...
public function ProductList(){
 ...
 public function save_upload_product_data(){
            $message = [];
            $config['upload_path']   = "./upload/product_csv/";
            $config['allowed_types'] = 'csv|xlsx';
            $config['max_size'] = 1024 * 20;
            
            $new_name = time().'_'.$_FILES["import_db"]['name'];
            $new_name = str_replace(" ","",$new_name);
            $new_name = str_replace("-","_",$new_name);
            $config['file_name'] = $new_name;
            $this->load->library('upload', $config);
            if(!$this->upload->do_upload('import_db')) 
                 {
            ...
            else 
               {
                ...
                $filePath1    = './upload/product_csv/';
                $filePath2    = $data['upload_data']['file_name'];
                $getcwd       = str_replace('\\', '/', getcwd());
                $filePath     = $getcwd.'/upload/product_csv/' . $config['file_name'];

               

                //$data["csv"]  = $this->csvreader->parse_file($filePath);
                
                //$job_data['user_id'] = $user_log;
                //$job_data['master_name'] = $this->input->post("master_name");
                //$job_data['file_name']   = $config['file_name'];
                
                // $this->base_model->insert_row_import("import_jobs",$job_data);
                //$job_id =$this->db->insert_id();

                  $enc = '"';
                  $sql = "LOAD DATA LOCAL INFILE '$filePath' INTO TABLE tbl_products FIELDS TERMINATED BY ',' ENCLOSED BY '$enc' LINES TERMINATED BY '\n' IGNORE 1 LINES (product_group,country_source,item_code,shape,range_collection,pattern_name,item_group,product_name,unit,mrp_price,sale_price,description,bullet1,bullet2,bullet3,bullet4,bullet5,warranty,image1,image2,image3,image4,image5,length,width,height,item_weight,number_pieces,cat_id,sub_cat_id,sub_cat_child_id);";
            
                
                  $query = $this->db->query($sql);
                  //$sql_update = "UPDATE " . $this->input->post("master_name") . "_tmp SET job_id  ='$job_id' WHERE  job_id IS NULL";
 ...
 public function save_product_data(){
   if($_POST){
    ...
    //$upload_file=$this->do_uploads();
      ...
      $image1=@$_POST['image'][0];
      $image2=@$_POST['image'][1];
      $image3=@$_POST['image'][2];
      $image4=@$_POST['image'][3];
      $image5=@$_POST['image'][4];

      $currenttime=date('Y-m-d h:i:s'); 
      $insert_data=array(
        'product_name'=>$this->input->post('product_name'),
        'material_id'=>$this->input->post('product_material'),
        'number_pieces'=>$this->input->post('number_pieces'),
        ...
        'range_collection'=>$this->input->post('range_collection'),
...
public function upload_product_csv(){
        ...
        $data['material_data']=$this->Product_model->get_material_data();
    
         //print_r($data['brand_data']);die;
       ...
       //$data['single_material_data']=$this->Product_model->single_material_list($eid);
        ...
        $data['subview']="upload-product-csv";
        $this->load->view('layout/default',$data);
...
public function category(){
   ...
   public function add_category() {
	...
	//	$data['single_brand']=$this->Product_model->single_brand_list($eid);
		//print_r($data['single_cat_data']);die;
        }
		$data['brand_data']=$this->Product_model->get_brand_data();
        $data['subview']="add-categoery";
        $this->load->view('layout/default',$data); 
...
public function blog_update_do_upload(){
    ...
    $path=$this->config->item('base_Url').'upload/blog/';
    $config1['upload_path']=$path;
    $config1['allowed_types']='jpg|png|gif';
    $config1['min_hieght']='10';
    $config1['min_width']='10';
...
public function delete_data($table, $field, $delete_id, $section){
	...
	public function add_material(){
        ...
        $data['subview']="add-material";
        $this->load->view('layout/default',$data); 
	}
	
	public function material_list(){
        $data['material_data']=$this->Product_model->material_list();
        $data['subview']="material-list";
        $this->load->view('layout/default',$data);

  }
  
  public function save_material_data(){
		...
		if(!empty($material_id)){
			...
			$msg=$this->Product_model->update_material_data($update_data,$material_id);
			redirect(base_url().'Productmaster/material_list');
		  ...
		  $result=$this->Product_model->save_material_data($insert_data);
          redirect(base_url().'Productmaster/material_list');
 
     }
     redirect(base_url().'Productmaster/material_list',$data);
