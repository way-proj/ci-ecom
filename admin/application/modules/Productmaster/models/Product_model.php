...
class Product_model extends CI_Model{
 ...
 function update_status($update_data,$id){
	...
	public function update_all_fields($update_data){
      //$this->db->where('1');
      $res=$this->db->update('tbl_fields',$update_data);
      echo $this->db->last_query();
      return $res;
	} 
	public function single_user_list($u_id){
         $this->db->select("*");
    ...
    public function save_brand_data($insert_data){
         $res=$this->db->insert('brand_details',$insert_data);
         return $res;
     }
	public function save_company_data($insert_data){
    ...
    public function get_product_gallery_details($p_id){
   ...
   public function get_product_gallery_details2($cat_id){
    ...
    public function save_attribute_data($insert_data){
         $res=$this->db->insert('attribute_details',$insert_data);
         return $res;
     }
    public function save_attribute_group_data($insert_data){
         $res=$this->db->insert('attribute_allocation',$insert_data);
         return $res;
     } 
    public function save_sub_cat_data($insert_data){
    ...
    public function save_sub_child_cat_data($insert_data){
  ...
  $material_id=@$post_data['product_material'];
        ...
        for($j=0;$j<$cnt;$j++){
          if($barcode[$j]){
            ...
            $insert_data=array('product_id'=>$insertid,
                             ...
                             'width'=>$width[$j],
                             'height'=>$height[$j],
							               'product_material'=>$material_id[$j],
                             'color'=>$color[$j],
                             'size'=>$size[$j],
                             'weight'=>$weight[$j],
                             'storage'=>$storage[$j],
  ...
  $height=@$post_data['height'];
   ...
   function cat_list(){
         $this->db->select("category_details.*,brand_details.brand_name");
         $this->db->from("category_details");
         $this->db->join("brand_details","brand_details.id=category_details.store");
         $this->db->order_by("id","desc");
         //$this->db->where('status','1');
         $query=$this->db->get();
         $res=$query->result();
...
function get_att($att_id){
 ...
 function sub_cat_list(){
         ...
         $this->db->join("category_details",'category_details.id=sub_category_details.cat_id');
          $this->db->join("brand_details","brand_details.id=sub_category_details.store");
 ...
 function sub_child_cat_list(){
        ...
        $this->db->join("brand_details","brand_details.id=sub_child_category_details.store");
        $this->db->join("sub_category_details","sub_category_details.id=sub_child_category_details.sub_cat_id"); 
 ...
 function single_cat_list($cat_id){
	...
	function single_materials_list($mid){
         $this->db->select("*");
...
FROM
  ...
  LEFT JOIN `tbl_products` AS tp 
   ...
   // Code added by Vinod Kumar Pal on 27062020
    ...
    function single_material_list($mid){
         $this->db->select("*");
         $this->db->from("tbl_materials");
         $this->db->where('id',$mid);
         $query=$this->db->get();
   ...
   function update_material_data($update_data,$mid){
         $this->db->where('id',$mid);
         $res=$this->db->update('tbl_materials',$update_data);
        //echo $this->db->last_query();die;
         return $res;
     }
	public function save_material_data($insert_data){
         $res=$this->db->insert('tbl_materials',$insert_data);
         return $res;
     }
